...
class Header extends Component {
  ...
  render() {
    return (
      <TrackerContainer>
        ...
        <TrackerViewContainer>
          <AddBtn onClick={() => this._addDateModalShow()}>Add New Date</AddBtn>
  ...
  _listContainer() {
    return <TrackerListContainer>{this._list()}</TrackerListContainer>;
  }
      ...
      <ItemDate>
        ...
        >
          ...
          >
            ...
            <ItemDateDayName>
              {item.date.toLocaleDateString("default", { weekday: "long" })}
            </ItemDateDayName>
          </SwiperSlide>
    ...
    return (
      <TrackItemColumnContainer>
        ...
        >
          ...
          {[...Array.from(Array(24).keys())].map((itemOpt, index) => (
            <option value={itemOpt + 1} key={index}>
              {itemOpt + 1 > 12
                ? itemOpt - 12 + 1 + " PM"
                : itemOpt + 1 + " AM"}
            </option>
  ...
  _calenderModal() {
    return (
      <CalendarModalContainer
        style={{ display: this.state.dateViewUniq == null ? "none" : "" }}
      >
        <CalenderModalBodyContainer>
          <CalenderModalBody>
            <CalenderModalCloseBtn
  ...
  _addDateModal() {
    return (
      <AddDateModalContainer
        style={{ display: this.state.addDateModalShow == null ? "none" : "" }}
      >
        <AddDateModalContainerSub>
          <AddDateModalContainerSubSub>
            <CalenderModalCloseBtn
            ...
            <AddDateModalBody>
              <AddDateModalSection>
                ...
                <AddDateModalCalenderContainer>
                  <DatePicker
                    ...
                    selected={this.state.newDateVal.date}
                    onChange={(date: any) =>
                      this.setState({
                        newDateVal: {
              ...
              <AddDateModalSection>
                ...
                >
                  <option value={0}>Select time</option>
                  {[...Array.from(Array(24).keys())].map((itemOpt, index) =>
                    this.state.newDateVal.start < itemOpt + 1 ? (
                      <option value={itemOpt + 1} key={index}>
                        {itemOpt + 1 > 12
                          ? itemOpt - 12 + 1 + " PM"
                          : itemOpt + 1 + " AM"}
  ...
  async _slideChange(uniq: any, type: string) {
    let currentIndex: any = this.state.dateSlots.filter(
      (item: any) => uniq == item.uniq
    );
